...
import Flower.minima.system.network.NetClientReader;
...
//				sp.getAllNodes().add(0,new SyncPacket(node, block.isLessEqual(casc)));
			...
			if(getMainDB().getMainTree().getAsList().size()==0) {
				...
				//FOR NOW
				SimpleLogger.log("Sync Complete.. Current block : "+getMainDB().getMainTree().getChainTip());
				...
				if(cross.isEqual(MiniNumber.ZERO)) {
					SimpleLogger.log("IRREGULAR POW INTRO CHAIN. NO CROSSOVER BLOCK.. "+sp);
					return;
				}
				...
				SimpleLogger.log("Sync complete. "+totalreq+" requests made.. ");
			}
			...
			if(txpow == null) {
				...
				SimpleLogger.log("TXPOWREQUEST OF MISSING TXPOW "+txpowid);
			}else {
			...
			if(!txpow.isBlock() && !txpow.isTransaction()) {
				SimpleLogger.log("ERROR FAKE - not enough POW : "+txpow);
				//Fake ?
				return;
			}
			...
			if(!sigsok) {
				//Reject
				SimpleLogger.log("Invalid Signatures with TXPOW : "+txpow); 
				return;
			}
			...
			if(!trxok) {
				//Reject
				SimpleLogger.log("NET TXPOW NOT OK : "+txpow); 
				return;
			}
