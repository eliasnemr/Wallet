        ...
        if (f.value.name && f.value.name.length > 0 && f.value.amount && f.value.amount > 0) {
            ...
            this.api.createToken(this.data).then(function (res) {
                if (res.status === true) {
                    ...
                    _this.presentToast("Your token creation failed, please check your records", 'danger');
                }
            });
        }
        else {
