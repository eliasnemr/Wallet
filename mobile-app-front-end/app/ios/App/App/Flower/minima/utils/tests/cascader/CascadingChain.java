		...
		for(int i=0;i<=MAX_BLOCK_LEVEL;i++) {
			...
			for(Block blk : Blocks) {
				if(blk.getBlockLevel()>=i) {
					SimpleLogger.log(counter+") Base Block "+blk.mBlockLevels[0].mBlockNumber);
					counter++;
					blk.printLevels();
			...
			if(lastlevelblock < NUMBER_BLOCKS - 1) {
				...
				for(int extra=i-1;extra>=0;extra--) {
					SimpleLogger.log("**** Level down ****** : "+extra);
			...
			SimpleLogger.log("Total TX-POW "+powratio+"%");
			SimpleLogger.log("Size  "+blockratio+"%");
		}
