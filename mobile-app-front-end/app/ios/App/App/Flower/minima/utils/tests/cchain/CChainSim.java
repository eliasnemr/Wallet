package Flower.minima.utils.tests.cchain;
...
//		    System.out.flush();  
			...
			SimpleLogger.log("********  NEW BLOCK ADDED!! ***\n"+newBlock);
//			SimpleLogger.log("\n********  NEW BLOCK ADDED!! *** "+newBlock.getFinalLevel());
			SimpleLogger.log("OLD  : "+mainChain.toLevelChain());
			...
			SimpleLogger.log("NEW  : "+mainChain.toLevelChain());
			
			double powperc = ((double)mainChain.getTotalPOW() / (NUMBER_BLOCKS)) * 100;
			SimpleLogger.log("Total POW : "+powperc+"%");
			
			double perc = ((double)mainChain.getSize() / (NUMBER_BLOCKS)) * 100;
			SimpleLogger.log("Size : "+perc+"% [ "+mainChain.getSize()+" / "+NUMBER_BLOCKS+" ]");
