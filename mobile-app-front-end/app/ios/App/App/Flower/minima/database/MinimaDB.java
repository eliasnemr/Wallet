package Flower.minima.database;
...
import Flower.minima.database.coindb.java.JavaCoinDB;
...
import Flower.minima.database.txpowdb.TxPowDB;
import Flower.minima.database.txpowdb.java.JavaDB;
import Flower.minima.database.txpowtree.BlockTree;
import Flower.minima.database.txpowtree.BlockTreeNode;
import Flower.minima.database.txpowtree.CascadeTree;
...
import Flower.minima.system.backup.SyncPackage;
import Flower.minima.system.backup.SyncPacket;
import Flower.minima.system.bootstrap.GenesisTxPOW;
import Flower.minima.system.brains.ConsensusHandler;
import Flower.minima.system.brains.TxPOWChecker;
import Flower.minima.system.network.RamIntro;
import Flower.minima.system.tx.TXMiner;
...
public class MinimaDB {
	...
	public void DoGenesis() {
		...
		MMRData gendata = new MMRData(MiniByte.FALSE, gencoin);
...
//				MMRProof proof = getMainTree().getChainTip().getMMRSet().getProof(row.getMMREntry());
				...
				if(proof == null) {
					SimpleLogger.log("ERROR NULL PROOF "+row);
					return null;
				}
		...
		if(currentin.isLess(zAmount)) {
			//ERROR!
			SimpleLogger.log("ERROR Insufficient funds!! "+currentin);
			return null;
		}
	...
	public TxPOW getCurrentTxPow(Transaction zTrans, Witness zWitness) {
		...
		if(!zTrans.isEmpty()) {
			...
			//MUST be valid.. ?
			if(!valid) {
				SimpleLogger.log("ERROR: Your own transaction is invalid !?");
				return null;
			}
		}
		...
		for(TxPOWDBRow row : unused) {
			...
			if(txp.isTransaction()) {
				...
				if(valid) {
					...
					SimpleLogger.log("Removing invalid TXPOW.. "+txp);
				}
			}
		}
