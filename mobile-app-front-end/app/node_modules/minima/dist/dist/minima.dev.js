...
var Minima = {
  ...
  init: function init(callback) {
    //Log a little..
    Minima.log("Initialising.."); //Calculate MiniDAPP ID given HREF location
    ...
    if (!Minima.debug) {
      if (window.location.protocol.startsWith("http")) {
        ...
        var rpcPort = +window.location.port - 2;
    ...
    Minima.log("WCHOST  : " + Minima.wshost); //Any Parameters..
   ...
   */
  notify: function notify(message, bgcolor) {
    //Log it..
    Minima.log("Notify : " + message); //Show a little popup across the screen..
   ...
   */
  cmd: function cmd(minifunc, callback) {
   ...
   * Run SQL
   */
  sql: function sql(query, callback) {
  ...
  net: {
    //SERVER FUNCTIONS
    onInbound: function onInbound(port, onReceivecallback) {
      MINIMA_SERVER_LISTEN.push({
        "port": port,
    ...
    broadcast: function broadcast(port, text) {
      MinimaRPC("net", "broadcast " + port + " " + text);
    },
    //USER FUNCTIONS
    onOutbound: function onOutbound(hostport, onReceivecallback) {
      MINIMA_USER_LISTEN.push({
        "port": hostport,
        "callback": onReceivecallback
      });
    },
    connect: function connect(hostport) {
      MinimaRPC("net", "connect " + hostport);
    },
    disconnect: function disconnect(UID) {
      MinimaRPC("net", "disconnect " + UID);
    },
    send: function send(UID, text) {
  ...
  file: {
    ...
    save: function save(text, file, callback) {
    ...
    saveHEX: function saveHEX(hextext, file, callback) {
      MinimaRPC("file", "savehex " + file + " " + hextext, callback);
    },
    loadHEX: function loadHEX(file, callback) {
      MinimaRPC("file", "loadhex " + file, callback);
    },
    //Copy file..
    copy: function copy(file, newfile, callback) {
      MinimaRPC("file", "copy " + file + " " + newfile, callback);
    },
    //Rename a file in your folder
    move: function move(file, newfile, callback) {
      MinimaRPC("file", "move " + file + " " + newfile, callback);
    },
    //List the files in a directory
    list: function list(file, callback) {
      MinimaRPC("file", "list " + file, callback);
    },
    //Delete a File
    delete: function _delete(file, callback) {
  ...
  form: {
    ...
    params: function params(paramname) {
      return MINIMA_PARAMS[paramname];
    },
    //Return the GET parameter by scraping the location..
    getParams: function getParams(parameterName) {
      var result = null;
      var tmp = [];
      var items = location.search.substr(1).split("&");

      for (var index = 0; index < items.length; index++) {
        tmp = items[index].split("="); //console.log("TMP:"+tmp);
  ...
  util: {
    ...
    getBalance: function getBalance(tokenid) {
      ...
      for (var balloop = 0; balloop < ballen; balloop++) {
        if (Minima.balance[balloop].tokenid == tokenid) {
          ...
          var mempool = Minima.balance[balloop].mempool; //Is there unconfirmed money coming..
    ...
    checkAllResponses: function checkAllResponses(responses) {
    ...
    getStateVariable: function getStateVariable(statelist, port) {
...
function MinimaWebSocketListener() {
  Minima.log("Starting WebSocket Listener @ " + Minima.wshost); //Check connected
  ...
  MINIMA_WEBSOCKET.onopen = function () {
    //Connected
    Minima.log("Minima WS Listener Connection opened.."); //Now set the MiniDAPPID

    var uid = {
      "type": "minidappid",
      "minidappid": Minima.minidappid
    }; //Send your name.. set automagically but can be hard set when debugging

    MINIMA_WEBSOCKET.send(JSON.stringify(uid)); //Send a message
  ...
  MINIMA_WEBSOCKET.onmessage = function (evt) {
    ...
    if (jmsg.event == "newblock") {
      ...
      Minima.txpow = jmsg.txpow; //What is the info message
      ...
      //Set the New Balance
      Minima.balance = jmsg.balance; //What is the info message

      info = {
        "balance": jmsg.balance
      }; //Post it..
  ...
  MINIMA_WEBSOCKET.onclose = function () {
    Minima.log("Minima WS Listener closed... reconnect attempt in 10 seconds"); //Start her up in a minute..
  ...
  MINIMA_WEBSOCKET.onerror = function (error) {
    ...
    var err = JSON.stringify(error, ["message", "arguments", "type", "name", "data"]); // websocket is closed.
...
function httpGetAsync(theUrl, callback) {
  var logenabled = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  var xmlHttp = new XMLHttpRequest();

  xmlHttp.onreadystatechange = function () {
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
      //Always a JSON ..
      var rpcjson = JSON.parse(xmlHttp.responseText); //Do we log it..
...
function MinimaCreateNotification(text) {
  var bgcolor = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "#bbbbbb";
  //First add the total overlay div
  var notifydiv = document.createElement('div'); //Create a random ID for this DIV..

  var notifyid = Math.floor(Math.random() * 1000000000).toString(); //Details..
  ...
  notifydiv.style.height = "90"; //Regular or specific color
  ...
  notifydiv.style.border = "thick solid #222222"; //Add it to the Page

  document.body.appendChild(notifydiv); //Create an HTML window

  var notifytext = "<table border=0 width=400 height=90><tr>" + "<td style='width:400;height:90;font-size:16px;font-family:monospace;color:black;text-align:center;vertical-align:middle;'>" + text + "</td></tr></table>"; //Now get that element

  var elem = document.getElementById(notifyid);

  if (elem) {
    //Set the Text..
    elem.innerHTML = notifytext; //Fade in..

    elem.style.transition = "all 1s"; // reflow

    elem.getBoundingClientRect(); // it transitions!

    elem.style.opacity = "0.8";
    elem.style.right = "40"; //And create a timer to shut it down..
    ...
    setTimeout(function () {
      ...
      var displayObject = document.getElementById(notifyid);
